#pragma once
/**
 * Result<T, E>, Ok<T> and Err<E> - Result types from Rust in C++
 * Infrastructure to make exceptions (and their overhead) largely
 * unnecessary, especially for the default case of result checking.
 */
...
namespace cc {
    namespace types {
        ...
        // Ok() with no type makes sense for normal Result usage
        // this would be in fact the simple case of Result<(),...>
        // in Rust.
        template<>
        struct Ok<void> { };
    ...
    namespace details {
        // type deduction from function/lambda or class method values
            ...
            namespace impl {
                ...
                struct Map <Ret(void)> {
                    ...
                    static Result<Ret, E> map(const Result<T, E>& result, Func func) {
                        static_assert(std::is_same<T, void>::value,
                            "Cannot map a void callback onto a non-void Result");
                ...
                struct Map<Result<U, E>(Arg)> {
                    ...
                    static Result<U, E> map(const Result<T, E>& result, Func func) {
                        static_assert(
                            std::is_same<T, Arg>::value || std::is_convertible<T, Arg>::value,
                            "Incompatible types for map argument and Result");
                        if (result.isOk()) {
                            return func(result.storage().template get<T>());
                        }
                        return types::Err<E>(result.storage().template get<E>());
                    }
                };
                ...
                struct Map<Result<U, E>(void)> {
                    template <typename T, typename Func>
                    static Result<U, E> map(const Result<T, E>& result, Func func) {
                        static_assert(std::is_same<T, void>::value, "Cannot map a void callback onto a non-void Result");
                        if (result.isOk()) {
                            return func();
                        }
                        return types::Err<E>(result.storage().template get<E>());
        ...
        namespace And {
            namespace impl {
                ...
                struct Then<Ret(void)> {
                    ...
                    static Result<T, E> then(const Result<T, E>& result, Func func) {
                        static_assert(std::is_same<T, void>::value,
                            "Cannot call a void callback onto a non-void Result");
                ...
                struct Else<Result<T, F>(void)> {
                    ...
                    static Result<T, F> orElse(const Result<T, E>& result, Func func) {
                        static_assert(
                            ...
                            "Cannot use a void callback with a non-void Result");
                        if (result.isErr()) {
                            auto res = func();
                            return res;
                        }
    ...
    template <typename T, typename E>
    struct Result {
        static_assert(!std::is_same<E, void>::value, "Result error type cannot be void");
        ...
        typename std::enable_if<
            ...
            U
        >::type unwrapOr(const U& defaultValue) const {
            if (isOk()) {
        ...
        E unwrapErr() const {
            ...
            std::fprintf(stderr, "Attempting to unwrapErr an error Result");
            std::terminate();
        }
    ...
    bool operator==(const Result<T, E>& lhs, const Result<T, E>& rhs) {
        static_assert(concept::EqualityComparable<T>::value, "Type for Ok() must be EquallyComparable for Result to be comparable");
        static_assert(concept::EqualityComparable<E>::value, "Type for Err() must be EquallyComparable for Result to be comparable");
